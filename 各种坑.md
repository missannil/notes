...
### tsc 报错 但是点开文件不报错 
载A文件中引入第三方库（annil时）写成了这样 import { SubComponent } from "annil/src/api/SubComponent";
这导致 tsc检查时 使用本地的tsconfig.json去编译SubComopnent，报错，但是打开SubComopnent文件时，ts插件类型提示会读取它所在上层目录的tsconfig文件，这导致不报错。